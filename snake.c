...
int in_game = 1;
int last_move = 0;
int score = 1; //score funciona como el tamaño de la serpiente
int x_coord = 0;
int y_coord = 0;
...
void eatApple() {
    if (isAppleEaten()) { //verifica si estamos arriba de una manzana
         // aumenta la longitud de la serpiente solo si no ha alcanzado el tamaño máximo
        if (score < led_matrix_height * led_matrix_width) {
            score++; //score funciona como el tamaño de la serpiente
        }
...
void generateApple(){ //genera la manzana
    ...
    // asigna la posición de la manzana y la pinta de verde
...
// Verifica si las coordenadas (x, y) están fuera del tablero
int isOutsideBoard(int x, int y) {
...
void updateSnake()
{
    if (score) { //verifica que la serpiente exista
        for (int i = score - 1; i > 0; i--) {
            // Guarda la posición actual de la cabeza como el siguiente segmento del cuerpo
    ...
    // actualiza la cabeza a la nueva posición
    ...
    for (int i = 1; i < score; i++) {
        if (snake_head == snake_body[i]) {
            in_game = 0; // si la serpiente se ha tocado a sí misma, el juego termina
            return;
        }
    }
    ...
    *snake_head = 0xFF0000;
    ...
    // si la longitud de la serpiente es mayor que el puntaje, apaga el último led
    if (score) {
...
{
    //antes de mover la serpiente, apaga el último segmento del cuerpo
    if (score) {
...
void wait(int ciclos) //wait casero
{
